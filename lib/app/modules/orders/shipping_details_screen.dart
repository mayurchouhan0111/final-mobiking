...
class ShippingDetailsScreen extends StatelessWidget {
  ...
  int _getCurrentMilestoneIndex(List<Scan> scans) {
    ...
    if (latestStatus.contains('OUT_FOR_DELIVERY') ||
        latestSrStatusLabel.contains('OUT FOR DELIVERY')) {
      return 3; // Out for Delivery
    }
    if (latestStatus.contains('IN_TRANSIT') ||
        latestSrStatusLabel.contains('IN TRANSIT')) {
      return 2; // In Transit
    }
    if (latestStatus.contains('SHIPPED') ||
        latestStatus.contains('PICKED_UP')) {
      return 1; // Shipped
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (currentMilestoneIndex == 4 && lastScanDateTime != null) {
      estimatedDelivery =
          'on ${DateFormat('EEEE, MMM d').format(lastScanDateTime.toLocal())}';
    }
    ...
    if (scans.isNotEmpty) {
      ...
      for (final scan in scans.reversed) {
        ...
        final String formattedDate = DateTime.tryParse(scan.date) != null
            ? DateFormat(
                'dd MMM, hh:mm a',
              ).format(DateTime.parse(scan.date).toLocal())
            : 'N/A';

        timelineMilestones.add(
          am_milestone.Milestone(
            isActive:
                true, // All history items are "active" in the timeline sense
            icon: const Icon(
              Icons.circle,
              color: AppColors.textLight,
              size: 12,
