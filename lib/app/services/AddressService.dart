...
class AddressService extends GetxService {
    ...
    try {
      ...
      if (response.statusCode == 200 && body['success'] == true) {
        if (body['data'] is List) {
          ...
          print(
            'AddressService: "data" field is not a List as expected: $body',
          );
          throw AddressServiceException(
            'Unexpected data format for addresses.',
          );
        }
      } else {
      ...
      if (e.response != null) {
        final errorMessage =
            ...
            'Server error during address fetch.';
        print(
          'AddressService: DioError during fetch: ${e.response?.statusCode} - $errorMessage',
        );
        throw AddressServiceException(
          errorMessage,
    ...
    try {
      ...
      if ((response.statusCode == 200 || response.statusCode == 201) &&
        ...
        if (userData != null &&
          ...
          print(
            'AddressService: Address added successfully (parsed from list): ${newAddressJson['_id']}',
          );
          return AddressModel.fromJson(newAddressJson);
        } else {
          print(
            'AddressService: Add response data missing address list or empty: $body',
          );
          throw AddressServiceException(
            'Address added but response data format unexpected.',
          );
        }
      } else {
        ...
        print(
          'AddressService: Failed to add address. Status ${response.statusCode}, Body: $body',
        );
        throw AddressServiceException(
          errorMessage,
          statusCode: response.statusCode,
      ...
      if (e.response != null) {
        final errorMessage =
            e.response?.data?['message'] ?? 'Server error during address add.';
        print(
          'AddressService: DioError during add: ${e.response?.statusCode} - $errorMessage',
        );
        throw AddressServiceException(
          errorMessage,
          statusCode: e.response?.statusCode,
  ...
  Future<AddressModel?> updateAddress(
    ...
    try {
      ...
      print(
        'AddressService: Updating address at $updateUrl with data: ${updatedAddress.toJson()}',
      );
      ...
      final response = await _dio.put(
        ...
        ), // Send the full updated address object
      );
      ...
      if (response.statusCode == 200 && body['success'] == true) {
        ...
        final Map<String, dynamic>? updatedAddressJson =
        ...
        if (updatedAddressJson != null) {
          ...
          print(
            'AddressService: Updated address data is null or not a Map: $body',
          );
          throw AddressServiceException(
            'Updated address data missing or incorrect format from response.',
          );
        }
      } else {
        ...
        print(
          'AddressService: Failed to update address. Status ${response.statusCode}, Body: $body',
        );
        throw AddressServiceException(
          errorMessage,
          statusCode: response.statusCode,
      ...
      if (e.response != null) {
        final errorMessage =
            e.response?.data?['message'] ??
            'Server error during address update.';
        print(
          'AddressService: DioError during update: ${e.response?.statusCode} - $errorMessage',
        );
        throw AddressServiceException(
          errorMessage,
          statusCode: e.response?.statusCode,
  ...
  Future<bool> deleteAddress(String addressId) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && body['success'] == true) {
        ...
        print(
          'AddressService: Failed to delete address. Status ${response.statusCode}, Body: $body',
        );
        throw AddressServiceException(
          errorMessage,
          statusCode: response.statusCode,
      ...
      if (e.response != null) {
        final errorMessage =
            e.response?.data?['message'] ??
            'Server error during address deletion.';
        print(
          'AddressService: DioError during delete: ${e.response?.statusCode} - $errorMessage',
        );
        throw AddressServiceException(
          errorMessage,
          statusCode: e.response?.statusCode,
