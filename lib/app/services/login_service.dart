...
class LoginService extends GetxService {
  ...
  Future<dio.Response> sendOtp(String phoneNumber) async {
    try {
      ...
      _log(
        'Send OTP response: Status ${response.statusCode}, Data: ${response.data}',
      );
      ...
      if (response.statusCode == 200 &&
        ...
        final errorMessage =
            responseData?['message'] ?? 'Failed to send OTP from backend.';
  ...
  Future<dio.Response> login(String phone, String otp) async {
    try {
      ...
      _log(
        'Login request data: {"mobile": "$phone", "role": "user", "otp": "$otp"}',
      );
      final response = await _dio.post(
        '$_baseUrl/login',
        data: {'phoneNo': phone, 'role': 'user', 'otp': otp},
        options: dio.Options(headers: {'Content-Type': 'application/json'}),
      );
      ...
      if (e.response != null) {
        ...
        _log(
          'Dio error during login: ${e.response?.statusCode} - $errorMessage',
        );
        throw LoginServiceException(
          errorMessage,
          statusCode: e.response?.statusCode,
  ...
  Future<dio.Response> logout() async {
    try {
      ...
      if (accessToken == null) {
        _log(
          'No access token found for logout. User is already considered logged out locally.',
        );
        throw LoginServiceException(
          'Access token not found. User is not logged in locally.',
        );
      }
      ...
      if (response.statusCode == 200) {
        ...
        final errorMessage =
            response.data?['message'] ?? 'Logout failed on server.';
        throw LoginServiceException(
          errorMessage,
          statusCode: response.statusCode,
        );
      ...
      if (e.response != null) {
        ...
        _log(
          'Dio error during logout: ${e.response?.statusCode} - $errorMessage',
        );
        throw LoginServiceException(
          errorMessage,
          statusCode: e.response?.statusCode,
  ...
  Future<dio.Response> deleteUser() async {
    try {
      ...
      if (accessToken == null) {
        _log(
          'No access token found for user deletion. User must be logged in.',
        );
        throw LoginServiceException(
          'Access token not found. Please log in first.',
        );
      }
      ...
      _log(
        'Delete user response: Status ${response.statusCode}, Data: ${response.data}',
      );
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        final errorMessage =
            response.data?['message'] ?? 'Failed to delete user account.';
        throw LoginServiceException(
          errorMessage,
          statusCode: response.statusCode,
        );
      ...
      if (e.response != null) {
        ...
        if (statusCode == 401) {
          ...
          throw LoginServiceException(
            'Permission denied. Cannot delete account.',
            statusCode: statusCode,
          );
        } else {
        ...
        throw LoginServiceException(
          'Network error during user deletion: ${e.message}',
        );
      }
    } catch (e) {
      _log('Unexpected error during user deletion: $e');
      throw LoginServiceException(
        'An unexpected error occurred during user deletion: $e',
      );
    }
  }
